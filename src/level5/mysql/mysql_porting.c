...
{ "ER_UNUSED_9", 1076, "You should never see it" },
...
{ "ER_SLAVE_SKIP_NOT_IN_GTID", 1966, "When using GTID, @@sql_slave_skip_counter can not be used. Instead, setting @@gtid_slave_pos explicitly can be used to skip to after a given GTID position." },
...
{ "ER_UNUSED_17", 1972, "You should never see it" },
...
{ "ER_INTERNAL_UNSUPPORT_SQL", ER_INTERNAL_UNSUPPORT_SQL, "myproxy error: SQL '%s' NOT support" },
{ "ER_INTERNAL_NO_SUCH_TBL", ER_INTERNAL_NO_SUCH_TBL, "myproxy error: no such table: %s.%s" },
{ "ER_INTERNAL_NO_MORE_TASK", ER_INTERNAL_NO_MORE_TASK, "myproxy error: can NOT allocate more tasks" },
...
char* find_mysqld_error(const int c)  
{
  for (size_t i=0;i<NUM_MYSQLD_ERRORS;i++) 
    if (g_mysqldError[i].code==c) {
      return (char*)g_mysqldError[i].desc ;
    }
...
char* find_sql_state(const int st)
{
  for (size_t i=0;i<NUM_SQL_STATES;i++) 
    if (g_sqlState[i].code==st) {
      return (char*)g_sqlState[i].desc ;
    }
